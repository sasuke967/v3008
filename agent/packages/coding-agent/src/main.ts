...
export async function main(args: string[]) {
	...
	if (mode === "rpc") {
		...
		if (solverLikePrintRun && exitCode === 0) {
			...
			// v35: post-prompt diff verification — diagnostic trace that we
			// checked whether the LLM actually produced any changes.
			try {
				const diffNames = execSync(
					...
					{ cwd: process.cwd(), timeout: 5_000, encoding: "utf-8" },
				).trim();
				const changedFiles = diffNames ? diffNames.split("\n").filter(Boolean).length : 0;
				if (changedFiles === 0) {
					console.error(chalk.yellow("[v35 audit] WARNING: post-prompt diff is empty — LLM produced no changes"));
				} else {
					console.error(chalk.dim(`[v35 audit] post-prompt diff: ${changedFiles} file(s) changed`));
				}
			} catch {}
		}
		stopThemeWatcher();
